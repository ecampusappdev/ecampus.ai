...
const Chat = ({ onSubmit, isInChatMode = false }) => {
  ...
  return (
    <div className={`w-full flex flex-col items-center gap-4 ${isInChatMode ? 'max-w-4xl mx-auto' : 'max-w-xl'}`}>
      {/* Input */}
      <form onSubmit={handleSubmit} className="relative w-full">
        <div className="relative bg-neutral-900 rounded-[10px] px-4 py-2 border border-blue-900 focus-within:border-blue-400 transition-all duration-200 flex items-start">
          <textarea
            ref={textareaRef}
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          ...
          <button
            type="submit"
            aria-label="Send"
            className="ml-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full p-2.5 transition-colors flex-shrink-0"
            style={{ alignSelf: query.includes('\n') ? 'flex-end' : 'center' }}
          >
            <svg
              className="w-4 h-4"
              fill="none"
